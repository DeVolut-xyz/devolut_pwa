...
export function WalletDetailPage() {
  ...
  const creditPositions = useMemo(() => {
    return tokens.filter(
      (t) => t.type === 'credit' || t.type === 'supply',
    )
  }, [tokens])

  const debtPositions = useMemo(() => {
    return tokens.filter((t) => t.type === 'debt')
  }, [tokens])
          ...
          <LiquidCard variant="dark">
            ...
            {aggregated.hasCreditOrDebt && (
              <VaultCreditDebt
                ...
                creditPositions={creditPositions.map((pos) => {
                  const tokenKey = buildTokenLogoKey({
                    chainId: pos.chainId,
                    address: pos.metadata.address,
                    tokenKey: pos.key,
                  })
                  const logoUrl =
                    (tokenKey ? getLogo(tokenKey) : undefined) ??
                    getLogo(pos.metadata.symbol)
                  return {
                    valueUsd: pos.value_usd,
                    symbol: pos.metadata.symbol,
                    apy: pos.apy,
                    logoUrl,
                  }
                })}
                debtPositions={debtPositions.map((pos) => {
                  const tokenKey = buildTokenLogoKey({
                    chainId: pos.chainId,
                    address: pos.metadata.address,
                    tokenKey: pos.key,
